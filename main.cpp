...
#include <cstdlib>
#include <cstdio>
#include <csetjmp> /* required for error handling */
    ...
    if (argc == 4) {
        ...
        if (f != NULL) {
            ...
            if (png_ptr != NULL) {
                ...
                double *filegamma = new double(-1);
                png_get_gAMA(png_ptr, info_ptr, filegamma);
                int srgb_intent = -1;
                png_get_sRGB(png_ptr, info_ptr, &srgb_intent);
                printf("gamma is %f\n", *filegamma);
                printf("sRGB intent is %d\n", srgb_intent);
                if (info_ptr != NULL) {
                    ...
                    if (setjmp(png_jmpbuf(png_ptr)) == 0) {
                         ...
                         */
                        row = (png_bytep) png_malloc(png_ptr, png_get_rowbytes(png_ptr,
                                                                               info_ptr));
